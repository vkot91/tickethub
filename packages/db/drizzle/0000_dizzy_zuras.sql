...
--> statement-breakpoint
DO $$ BEGIN
 CREATE TYPE "shows"."seat_tier" AS ENUM('vip', 'standard', 'economy');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "shows"."ticket_types" (
	...
	"section_id" uuid,
	"name" text NOT NULL,
	"tier" "shows"."seat_tier" DEFAULT 'standard' NOT NULL,
...
CREATE TABLE IF NOT EXISTS "fulfillment"."tickets" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "tickets_order_seat_uq" UNIQUE("order_id","seat_id")
);
--> statement-breakpoint
DO $$ BEGIN
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "shows"."ticket_types" ADD CONSTRAINT "ticket_types_section_id_sections_id_fk" FOREIGN KEY ("section_id") REFERENCES "shows"."sections"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
