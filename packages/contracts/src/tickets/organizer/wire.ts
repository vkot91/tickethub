...
/**
 * The console's surface onto `apps/tickets` — the dashboard's check-in number, and the gate
 * scanner. Both are scoped by a resolved `showIds[]`, like every other organizer read: the gateway
 * proves ownership first, so nothing in Tickets ever learns who owns what.
 */
...
/**
 * A scan on the wire. Identical to what the gateway answers: Tickets owns the ticket, so it
