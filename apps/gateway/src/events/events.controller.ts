...
const keys = MESSAGE_PATTERNS.events;
...
export class GatewayEventsController {
  ...
  @Get() catalog(@Query() query: unknown) {
    return firstValueFrom(this.events.send(keys.catalog, catalogQuerySchema.parse(query)));
  }

  @Get(':id') detail(@Param('id') id: string) {
    return firstValueFrom(this.events.send(keys.detail, { id }));
  }

  @Get(':id/seat-map') seatMap(@Param('id') id: string) {
    return firstValueFrom(this.events.send(keys.seatMap, { id }));
  }
}
