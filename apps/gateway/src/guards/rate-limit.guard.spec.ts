...
describe('RateLimitGuard', () => {
  it('allows requests under the limit', async () => {
    ...
    await expect(new RateLimitGuard(redis as any).canActivate(ctx())).resolves.toBe(true);
  ...
  it('throws 429 once the limit is exceeded', async () => {
    ...
    await expect(new RateLimitGuard(redis as any).canActivate(ctx())).rejects.toThrow(
      HttpException,
    );
  });
});
