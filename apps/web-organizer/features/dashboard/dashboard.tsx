...
import { fetchOrganizerShows, fetchRecentOrders, fetchShowStats, organizerKeys } from '../api';
...
export function Dashboard() {
  ...
  const showId = selectedShowId ?? shows?.items[0]?.id;

  const { data: stats } = useQuery({
    queryKey: organizerKeys.stats(showId ?? ''),
    queryFn: () => fetchShowStats(showId!),
    enabled: Boolean(showId),
  });

  const { data: recentOrders } = useQuery({
    queryKey: organizerKeys.recentOrders(),
    queryFn: fetchRecentOrders,
  });

  if (isLoadingShows) return <Skeleton className="h-105 rounded-panel" />;

  if (!shows || shows.items.length === 0) {
    return (
    ...
    <>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <Select
          ...
          options={shows.items.map((show) => ({ value: show.id, label: show.title }))}
        />
      </div>
