 ...
 * for one show, and across several it would be adding up different `ticket_types` rows that happen
 * to share a name. The caller decides that; this only renders what it is handed.
 */
export function TicketTypesCard({ byTier, soldCount }: Pick<ShowStats, 'byTier' | 'soldCount'>) {
  return (
    <Card padding="lg" asChild>
      <section aria-labelledby="ticket-types-heading">
        <h2 id="ticket-types-heading" className="mb-5 font-display text-lg font-semibold">
          Ticket types
        </h2>
