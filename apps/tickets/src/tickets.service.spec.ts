...
const order: OrderResponse = {
  ...
  seats: [
    { seatId: KNOWN_SEAT_ID, bandId: BAND_ID },
    { seatId: UNKNOWN_SEAT_ID, bandId: BAND_ID },
  ],
};
      ...
      rows: [
        {
          ...
          number: 1,
          seats: [{ id: KNOWN_SEAT_ID, number: 3, bandId: null, priceCents: null, tier: null }],
        },
      ],
    },
  ],
...
interface FakeOptions {
  // Message ids already committed to tickets.processed_messages.
  ...
  // Rows the read paths (list / pdfUrl) should find.
...
function makeFakes({
  ...
  const rowsMatching = (table: unknown, condition: SQL) => {
    if (table === ticketsProcessedMessages) {
...
describe('TicketsService.issue', () => {
  ...
  it('is a no-op when the message was already processed', async () => {
    ...
    await serviceFor(fakes).issue(orderPaid);
  ...
  it('pre-checks the inbox with a read-only select, never a claiming insert', async () => {
    ...
    expect(fakes.selectedTables).toEqual([ticketsProcessedMessages]);
    expect(fakes.selectedConditions).toEqual([eq(ticketsProcessedMessages.messageId, MESSAGE_ID)]);
  ...
  it('falls back to a generic tier when no price tier matches the price band', async () => {
    const fakes = makeFakes();
  ...
  it('enqueues ticket.pdf_ready once for the order, not once per seat', async () => {
    ...
    // consumer downstream. The stamp itself is OutboxRepository's own spec.
  ...
  it('does not re-emit ticket.pdf_ready when the order already has its tickets', async () => {
    ...
    await serviceFor(fakes).issue({ ...orderPaid, messageId: OTHER_MESSAGE_ID });
  ...
  it('uses different ticket ids for the same seat in a different order', async () => {
    ...
    await serviceFor(fakes).issue({ ...orderPaid, orderId: SECOND_ORDER_ID });
  ...
  it('replays a failed delivery: the retry mints exactly one set of tickets and one outbox row', async () => {
    ...
    await expect(service.issue(orderPaid)).rejects.toThrow('s3 blip');
  ...
  it('propagates an s3 failure so the message is nacked, without writing the tickets', async () => {
    ...
    await expect(serviceFor(fakes).issue(orderPaid)).rejects.toThrow('s3 down');
  ...
  it('propagates an rpc failure so the message is nacked, without writing the tickets', async () => {
    ...
    await expect(serviceFor(fakes).issue(orderPaid)).rejects.toThrow('orders rpc down');
  ...
  it('propagates a pdf render failure so the message is nacked, without writing the tickets', async () => {
    ...
    await expect(serviceFor(fakes).issue(orderPaid)).rejects.toThrow('pdf render failed');
...
describe('TicketsService.list', () => {
  it('scopes the query to the caller', async () => {
    ...
    await serviceFor(fakes).list(USER_ID);
  ...
  it('returns nothing for a user with no tickets, without calling shows', async () => {
    ...
    const list = await serviceFor(fakes).list(OTHER_USER_ID);
  ...
  it('joins the live show title and start time rather than a snapshot', async () => {
    ...
    const { items } = await serviceFor(fakes).list(USER_ID);
...
describe('TicketsService.pdfUrl', () => {
  it('mints a short-lived signed url for the ticket’s object', async () => {
    const fakes = makeFakes({ existingTickets: [storedTicket()] });

    const { url } = await serviceFor(fakes).pdfUrl(USER_ID, TICKET_ID_KNOWN_SEAT);
  ...
  it('names the download after the ticket', async () => {
    ...
    await serviceFor(fakes).pdfUrl(USER_ID, TICKET_ID_KNOWN_SEAT);
  ...
  it('refuses another user’s ticket without confirming it exists', async () => {
    ...
    await expect(serviceFor(fakes).pdfUrl(OTHER_USER_ID, TICKET_ID_KNOWN_SEAT)).rejects.toThrow(
      'Ticket not found',
    );

    await expect(serviceFor(fakes).pdfUrl(OTHER_USER_ID, TICKET_ID_SECOND_ORDER)).rejects.toThrow(
      'Ticket not found',
    );
