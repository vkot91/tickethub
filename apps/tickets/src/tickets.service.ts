...
import { tickets, ticketsProcessedMessages, type Db } from '@tickethub/db';
...
export class TicketsService {
  ...
  async issue(event: EventEnvelope<typeof ORDER_ROUTING_KEYS.ORDER_PAID>): Promise<void> {
    ...
    const [alreadyCommitted] = await this.db
      .select({ messageId: ticketsProcessedMessages.messageId })
      .from(ticketsProcessedMessages)
      .where(eq(ticketsProcessedMessages.messageId, event.messageId))
      .limit(1);
    ...
    const tierNameByBandId = new Map(
    ...
    const seatTickets = order.seats.map((seat) => {
      ...
      return {
        ...
        tier: tierNameByBandId.get(seat.bandId) ?? 'Standard',
  ...
  async list(userId: string): Promise<TicketList> {
   ...
   */
  async pdfUrl(userId: string, ticketId: string): Promise<TicketPdfUrl> {
