 ...
 * number. `showIds` always arrive resolved by the gateway — nothing here checks ownership, and an
 * empty list is zero rather than "every show on the platform".
 *
 * A scan is scoped to **one** show, the door the attendant is standing at: scoped to every show an
 * organizer owns it would admit next week's ticket tonight and burn it.
 */
@Injectable()
export class OrganizerCheckInService {
  ...
  private async rejected(result: 'invalid' | 'wrongShow', showId: string): Promise<CheckInScan> {
    return {
      ...
      seatLabel: null,
      checkedInAt: null,
      checkedInCount: await this.gateCount(showId),
    };
  }
  ...
  /** The dashboard's number: seats through the gate across every show the caller owns. */
  ...
  private async gateCount(showId: string): Promise<number> {
    const [counted] = await this.db
      .select({ count: sql<number>`count(*)::int` })
      .from(tickets)
