...
Tickets microservice: turns a paid order into QR-stamped PDF tickets in object storage, emails
them to the buyer, and serves the buyer's ticket list. Owns `pgSchema('tickets')` (`tickets`,
...
- **Consumer** `order.paid` (queue `tickets.order-paid` + DLX) — renders and stores the ticket:
...
- **Consumer** `ticket.pdf_ready` (queue `tickets.pdf-ready` + DLX) — claims
...
| Var                               | What it is                                                                            |
| --------------------------------- | ------------------------------------------------------------------------------------- |
| `DATABASE_URL`                    | Postgres — the `tickets` schema                                                       |
...
#    auth, shows, orders, payments, gateway, tickets:
...
#    seat ids + bandId come from the seeded "Demo Concert (seated)" show:
...
curl -sX POST localhost:3000/orders -H "authorization: Bearer $TOKEN" \
  ...
  -d '{"showId":"<showId>","seats":[{"seatId":"<seatId>","bandId":"<bandId>"}]}'
...
- `tickets.tickets` → **exactly one** row, `id = order_id`,
...
- `tickets.outbox` → one `ticket.pdf_ready` row with `published_at` set.
- `tickets.processed_messages` → two rows (`order.paid` and `ticket.pdf_ready`).
